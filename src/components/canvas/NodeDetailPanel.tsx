 ...
 * One component for ALL node types — the tabs are computed from the node's
 * capabilities, never per-type (spec §61 "Avoid unique panel architecture per
 * node").
 *
 ...
 * `updateNodeData` (Configure tab edits) and `openNodeDetail`/`closeNodeDetail`.
 * `detailNodeId` is transient (NOT in partialize).
 */
    ...
    >
      <DialogContent side="right" width={480} aria-describedby={undefined}>
        {detailNodeId ? (
          ...
          // useState initializer) per node — no stale tab leaks when switching
          // between nodes, and the §52 Preview-node "open on Preview" shortcut
          // re-evaluates per node.
...
function NodeDetailBody({ nodeId, onClose }: { nodeId: string; onClose: () => void }) {
